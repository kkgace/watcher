...
public class BxMonitor {
    private static final Logger logger = LoggerFactory.getLogger(BxMonitor.class);

    private static Map<String, BxMonitor.MonitorItem> items = new ConcurrentHashMap();
    ...
    public static void recordMany(String name, long count, long time) {
        BxMonitor.MonitorItem item = items.get(name);
        if (item == null) {
    ...
    static {
        ...
        timer.scheduleAtFixedRate(new BxMonitor.MonitorTask(), start.getTime(), 30000L);
    }
    ...
    private static class MonitorTask extends TimerTask {
        ...
        public void run() {
            ...
            try {
                ...
                BxMonitor.items.forEach((name, items) -> {
                    BxMonitor.MonitorItem item = items.dumpAndClearItem();
                ...
                BxMonitor.values.forEach((key, value) ->
                        itemMap.put(this.makeName(key), value.get())
                );

                BxMonitor.currentItems = Collections.unmodifiableMap(itemMap);
